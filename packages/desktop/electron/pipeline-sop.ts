...
async function runSopPipeline(
  ...
  const inspect = await runSopInspectStage(id, runner, guard.guardReport);
...
export async function runSopJob(
  ...
  const report = await runSopPipeline(id, runner, dryRun);
...
export async function runFullPipelineJob(
  ...
  const guardReport = await runner.runGuard({ dryRun });
  if (guardReport.ok === false) {
