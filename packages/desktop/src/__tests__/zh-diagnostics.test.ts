...
function makeGuardReport(): GuardReport {
  return {
    ...
    results: [
      { checkId: 'eslint', adapter: 'eslint', status: 'failed', severity: 'error', blocking: true, message: 'no-any 违规' },
      { checkId: 'gitleaks', adapter: 'gitleaks', status: 'passed', severity: 'info', blocking: false, message: 'ok' },
    ],
        ...
        smells: [
          {
            ...
            severity: 'warning',
            message: '函数复杂度过高',
            location: { filePath: 'src/order.service.ts', line: 42, column: 3, endLine: 42, endColumn: 30 },
            context: { metric: 'cyclomatic', value: 12, threshold: 10 },
            suggestion: { type: 'extract', description: '拆分为多个小函数', priority: 'high', effort: 'medium', autoFixable: false },
          },
        ],
      },
    ],
    ...
    evaluations: [
      {
        ...
        files: ['src/main.ts'],
        violations: [
          { id: 'v-1', ruleId: 'security/no-eval', severity: 'high', file: 'src/main.ts', line: 8, column: 4, message: '禁止 eval', suggestion: '使用 JSON.parse' },
        ],
        durationMs: 5,
...
describe('buildDiagnosticsReport', () => {
  const issues: readonly DiagnosticEntry[] = [
    { ruleId: 'a/one', severity: 'error', category: 'quality', message: 'e1', file: 'src/a.ts', line: 1, column: 2, autoFixable: false, source: 'inspect', fingerprint: 'a/one:src/a.ts:1' },
    { ruleId: 'b/two', severity: 'warning', category: 'security', message: 'w1', file: 'src/b.ts', autoFixable: true, source: 'guard', fingerprint: 'b/two:src/b.ts:0' },
    { ruleId: 'c/three', severity: 'info', category: 'quality', message: 'i1', file: '', autoFixable: false, source: 'guard', fingerprint: 'c/three::0' },
  ];

  it('按文件分组生成 LSP 风格 files 视图，无位置的 issue 不进 files', () => {
    const report = buildDiagnosticsReport({ path: '/proj', name: 'proj' }, issues, '2026-07-31T00:00:00.000Z');
...
describe('writeDiagnosticsFile', () => {
  it('写入 .zhshield/diagnostics/latest.json 并返回绝对路径', () => {
    const project = mkdtempSync(join(tmpdir(), 'zh-diagnostics-'));
    try {
      const report = buildDiagnosticsReport({ path: project, name: 'tmp' }, [], '2026-07-31T00:00:00.000Z');
      const absPath = writeDiagnosticsFile(report);
