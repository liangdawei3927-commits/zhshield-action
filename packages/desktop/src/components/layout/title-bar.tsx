  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 pl-0" style={{ WebkitAppRegion: 'drag' }}>
        ...
        <ShieldLogo size={18} />
        <span style={{ fontSize: 'var(--zh-font-md)', fontWeight: 600, letterSpacing: 0.5 }}>{t('layout.brandName')}</span>
      </div>

      {/* 中：项目文件夹（居中） */}
      <div className="flex-1 flex items-center justify-center gap-2" style={{ WebkitAppRegion: 'drag' }}>
        {projectName ? (
          <>
          ...
          <span style={{ fontSize: 'var(--zh-font-sm)', opacity: 0.75 }}>{t('layout.noProject')}</span>
        )}
      </div>
