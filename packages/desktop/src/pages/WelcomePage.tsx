...
function WelcomeCorners({ maximized }: { maximized: boolean }) {
  ...
  return (
    <>
      ...
      {/* 右上角：引擎状态 */}
      <div className="absolute top-4 right-5 flex items-center gap-2 px-3 py-1.5 rounded-full" style={{ background: 'rgb(var(--zh-brand) / 0.12)', border: '1px solid rgb(var(--zh-brand) / 0.25)' }}>
        <span className="w-2 h-2 rounded-full" style={{ background: 'rgb(var(--zh-brand))', boxShadow: '0 0 6px rgb(var(--zh-brand) / 0.6)' }} />
        <span className="text-xs font-medium" style={{ color: 'rgba(255,255,255,0.7)' }}>{t('page.welcome.engineOn')}</span>
      </div>

      {/* 右下角 */}
      <div className="absolute bottom-4 right-5 flex items-center gap-2" style={{ color: 'rgba(255,255,255,0.35)' }}>
        <span className="text-xs flex items-center gap-1.5">
          <span className="w-1.5 h-1.5 rounded-full" style={{ background: 'rgb(var(--zh-brand))', boxShadow: '0 0 4px rgb(var(--zh-brand) / 0.5)' }} />
...
function WelcomeHero() {
  return (
    <div className="relative flex items-center justify-center mb-8" style={{ width: 200, height: 200 }}>
      <div className="absolute rounded-full" style={{ width: 200, height: 200, background: 'radial-gradient(circle, rgb(var(--zh-brand) / 0.25) 0%, rgb(var(--zh-brand) / 0.05) 60%, transparent 80%)' }} />
      <div className="absolute rounded-full" style={{ width: 180, height: 180, border: '2px solid rgb(var(--zh-brand) / 0.3)', borderTop: '2px solid rgba(255,255,255,0.6)', animation: 'spin 6s linear infinite' }} />
      <div className="absolute rounded-full" style={{ width: 140, height: 140, border: '1px solid rgb(var(--zh-brand) / 0.2)' }} />
...
function WelcomeStats() {
  ...
  const stats = [
    ...
    { label: t('page.welcome.stat.totalBlocked'), value: t('page.welcome.stat.times', { count: 0 }) },
