...
export function HealthRing({ score }: { score: number | null }) {
  ...
  const color = score === null
    ? 'rgb(var(--zh-success))'
    : progress === 100 ? 'rgb(var(--zh-success))'
    : progress >= 90 ? 'rgb(var(--zh-info))'
    : progress >= 75 ? 'rgb(var(--zh-brand-500))'
    : progress >= 60 ? 'rgb(var(--zh-warning))'
    : 'rgb(var(--zh-danger))';

  return (
    <div className="relative w-[150px] h-[150px] flex items-center justify-center">
      <svg width="150" height="150" viewBox="0 0 150 150" className="absolute">
        <circle cx="75" cy="75" r={radius} fill="none" stroke={score === null ? 'rgb(var(--zh-success))' : 'rgb(var(--zh-line))'} strokeWidth="8" opacity={score === null ? 0.3 : 1} />
        {score !== null && (
          <circle
      ...
      <div className="relative z-10 text-center">
        <div className="text-3xl font-bold" style={{ color }}>{score === null ? '--' : Math.round(progress)}</div>
        <div className="text-[10px] text-zh-muted mt-0.5">{score === null ? t('page.dashboard.ring.notChecked') : t('page.dashboard.ring.healthScore')}</div>
      </div>
    </div>
  );
}
...
function HomeCheckControls({ running, progressLabel, pipelineProgress, autoFixNotice, onCheck }: {
  running: boolean;
  ...
  return (
    <>
      ...
      <RunStatus running={running} pipelineProgress={pipelineProgress} autoFixNotice={autoFixNotice} />
    </>
  );
}

export function HomeView({ score, running, progressLabel, pipelineProgress, autoFixNotice, onCheck }: {
  score: number | null;
  running: boolean;
  progressLabel: string;
  pipelineProgress: PipelineProgress | null;
        ...
        <div className="flex-1 flex flex-col items-center justify-center min-h-0 w-full">
          <div className="flex items-center gap-12 mb-8">
            ...
            <div className="flex flex-col">
              ...
              <div className="text-sm text-zh-muted mt-2">
                {t('page.dashboard.guardSlogan')}
              </div>
            </div>
          </div>
...
function ProductIntro() {
  ...
  const axes = [
    ...
    },
    {
      ...
      icon: (
        <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          <rect x="4" y="3" width="16" height="18" rx="2" /><path d="M8 10h8M8 14h5" /><path d="M8 7.5l1 1 2-2" />
        </svg>
      ),
    },
    {
  ...
  return (
    <div
      ...
      style={{ background: 'linear-gradient(135deg, rgb(var(--zh-brand-900)) 0%, rgb(var(--zh-brand-hover)) 60%, rgb(var(--zh-brand-600)) 100%)', boxShadow: '0 8px 24px rgb(var(--zh-brand-900) / 0.18)' }}
    >
      <div className="flex items-center gap-4 min-w-0">
        ...
        <div className="min-w-0">
          <div className="text-xl font-bold text-white tracking-wide">{t('page.dashboard.productIntro.title')}</div>
          <div className="text-xs mt-1.5" style={{ color: 'rgba(255,255,255,0.65)' }}>{t('page.dashboard.productIntro.subtitle')}</div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 shrink-0">
        {axes.map((item) => (
          ...
          >
            ...
            <span>
              <span className="block text-base font-semibold text-white leading-none whitespace-nowrap">{item.label}</span>
              <span className="block text-sm mt-1.5 leading-none whitespace-nowrap" style={{ color: 'rgba(255,255,255,0.6)' }}>{item.desc}</span>
            </span>
          </div>
        ))}
      </div>
