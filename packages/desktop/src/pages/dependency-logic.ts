...
export const LICENSE_CATEGORY_CONFIG: Record<
  string,
  { color: string; bg: string; labelKey: string; riskKey: string }
> = {
  permissive: {
    color: 'rgb(var(--zh-success-700))',
    bg: 'rgb(var(--zh-success) / 0.08)',
    labelKey: 'page.deps.license.permissive',
    riskKey: 'page.deps.license.risk.low',
  },
  'weak-copyleft': {
    color: 'rgb(var(--zh-warning))',
    bg: 'rgb(var(--zh-warning) / 0.12)',
    labelKey: 'page.deps.license.weakCopyleft',
    riskKey: 'page.deps.license.risk.medium',
  },
  'strong-copyleft': {
    color: 'rgb(var(--zh-danger))',
    bg: 'rgb(var(--zh-danger) / 0.1)',
    labelKey: 'page.deps.license.strongCopyleft',
    riskKey: 'page.deps.license.risk.high',
  },
  unknown: {
    color: 'rgb(var(--zh-muted))',
    bg: 'rgb(var(--zh-muted) / 0.08)',
    labelKey: 'page.deps.license.unknown',
    riskKey: 'page.deps.license.risk.unknown',
  },
};

/** 许可证类别展示顺序（矩阵四宫格） */
export const LICENSE_CATEGORY_ORDER: string[] = [
  'permissive',
  'weak-copyleft',
  'strong-copyleft',
  'unknown',
...
export const TRUST_STATUS_CONFIG: Record<string, { color: string; bg: string; labelKey: string }> =
  {
    verified: {
      color: 'rgb(var(--zh-success-700))',
      bg: 'rgb(var(--zh-success) / 0.08)',
      labelKey: 'page.deps.trust.verified',
    },
    suspicious: {
      color: 'rgb(var(--zh-warning))',
      bg: 'rgb(var(--zh-warning) / 0.12)',
      labelKey: 'page.deps.trust.suspicious',
    },
    compromised: {
      color: 'rgb(var(--zh-danger))',
      bg: 'rgb(var(--zh-danger) / 0.1)',
      labelKey: 'page.deps.trust.compromised',
    },
    unknown: {
      color: 'rgb(var(--zh-muted))',
      bg: 'rgb(var(--zh-muted) / 0.08)',
      labelKey: 'page.deps.trust.unknown',
    },
  };
...
export const LOCKFILE_CHECKS: Array<{
  ...
  {
    key: 'present',
    okKey: 'page.deps.lockfile.present',
    failKey: 'page.deps.lockfile.notPresent',
    naKey: 'page.deps.lockfile.notEvaluable',
  },
  {
    key: 'consistent',
    okKey: 'page.deps.lockfile.consistent',
    failKey: 'page.deps.lockfile.notConsistent',
    naKey: 'page.deps.lockfile.notEvaluableConsistency',
  },
  {
    key: 'integrityVerified',
    okKey: 'page.deps.lockfile.integrityVerified',
    failKey: 'page.deps.lockfile.integrityFailed',
    naKey: 'page.deps.lockfile.notEvaluableIntegrity',
  },
];
...
export const RISK_BADGE_CONFIG: Record<string, { color: string; bg: string; labelKey: string }> = {
  high: {
    color: 'rgb(var(--zh-danger))',
    bg: 'rgb(var(--zh-danger) / 0.1)',
    labelKey: 'page.deps.risk.high',
  },
  medium: {
    color: 'rgb(var(--zh-warning))',
    bg: 'rgb(var(--zh-warning) / 0.12)',
    labelKey: 'page.deps.risk.medium',
  },
  low: {
    color: 'rgb(var(--zh-muted))',
    bg: 'rgb(var(--zh-muted) / 0.08)',
    labelKey: 'page.deps.risk.low',
  },
};
...
export const ENV_SEVERITY_CONFIG: Record<string, { color: string; bg: string; labelKey: string }> =
  {
    error: {
      color: 'rgb(var(--zh-danger))',
      bg: 'rgb(var(--zh-danger) / 0.1)',
      labelKey: 'page.deps.env.severity.error',
    },
    warning: {
      color: 'rgb(var(--zh-warning))',
      bg: 'rgb(var(--zh-warning) / 0.12)',
      labelKey: 'page.deps.env.severity.warning',
    },
    info: {
      color: 'rgb(var(--zh-muted))',
      bg: 'rgb(var(--zh-muted) / 0.08)',
      labelKey: 'page.deps.env.severity.info',
    },
  };
