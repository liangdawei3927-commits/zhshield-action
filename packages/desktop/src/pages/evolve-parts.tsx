...
export function NodeGraph() {
  return (
    <svg width="150" height="150" viewBox="0 0 150 150" fill="none">
      ...
      <line x1="105" y1="70" x2="90" y2="105" stroke="rgb(var(--zh-info) / 0.2)" strokeWidth="1.2" />
...
export function EvolveHeader({ suggestionCount, weightCount, loading, onRefresh }: { suggestionCount: number; weightCount: number; loading: boolean; onRefresh: () => void }) {
  const t = useT();
  return (
    <div className="flex items-center gap-4 mb-8">
      <Bounce className="w-12 h-12 rounded-xl bg-cyan-50 flex items-center justify-center">
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="rgb(var(--zh-info))" strokeWidth="1.8">
          <circle cx="12" cy="12" r="3" /><path d="M19.4 15a1.65 1.65 0 00.33 1.82l.06.06a2 2 0 010 2.83 2 2 0 01-2.83 0l-.06-.06" />
      ...
      <div>
        ...
        <p className="text-sm text-zh-muted">{t('page.evolve.header.count', { suggestions: suggestionCount, weights: weightCount })}</p>
      </div>
      <PrimaryButton className="ml-auto" onClick={onRefresh} loading={loading} loadingLabel={t('common.loading')}>
        {t('common.refresh')}
      </PrimaryButton>
    </div>
  );
...
export function EvolveStats({ suggestions, weights, adjustedCount, highFp }: { suggestions: number; weights: number; adjustedCount: number; highFp: number }) {
  const t = useT();
  return (
    <div className="flex gap-4 mb-6">
      {[
        { labelKey: 'page.evolve.stats.suggestions', value: String(suggestions), color: 'rgb(var(--zh-info))' },
        { labelKey: 'page.evolve.stats.weights', value: String(weights), color: 'rgb(var(--zh-info))' },
        { labelKey: 'page.evolve.stats.adjusted', value: String(adjustedCount), color: 'rgb(var(--zh-warning))' },
        { labelKey: 'page.evolve.stats.highFp', value: String(highFp), color: 'rgb(var(--zh-danger))' },
      ].map((stat) => (
...
export function SuggestionsPanel({ suggestions }: { suggestions: SuggestionData[] }) {
  ...
  return (
    <div className="flex-1">
      <h3 className="text-sm font-semibold text-zh-ink-2 mb-3">{t('page.evolve.stats.suggestions')}</h3>
      <div className="space-y-2">
        {suggestions.map((s) => (
          <BounceCard key={s.ruleId} className="p-3 rounded-lg bg-zh-panel">
            <div className="flex items-center gap-2 mb-1">
              <span className="text-xs font-mono px-1.5 py-0.5 rounded bg-blue-50 text-blue-600">{s.ruleId}</span>
              <span className="text-xs text-zh-muted">{t('page.evolve.suggestion.confidence', { confidence: Math.round(s.confidence * 100) })}</span>
            </div>
...
export function WeightsPanel({ weights }: { weights: RuleWeightData[] }) {
  ...
  return (
    <div className="flex-1">
      ...
      <div className="space-y-2">
        {weights.map((w) => {
          const barColor = w.weight >= 0.8 ? 'rgb(var(--zh-success))' : w.weight >= 0.5 ? 'rgb(var(--zh-warning))' : 'rgb(var(--zh-danger))';
          return (
            <BounceCard key={w.ruleId} className="p-3 rounded-lg bg-zh-panel">
              ...
              <div className="w-full h-1.5 rounded-full bg-zh-line">
                <div className="h-full rounded-full transition-all" style={{ width: `${w.weight * 100}%`, background: barColor }} />
              </div>
              <div className="flex justify-between mt-1 text-xs text-zh-muted">
                <span>{t('page.evolve.weight.fpRate', { rate: (w.falsePositiveRate * 100).toFixed(0) })}</span>
...
export function EmptyAnalysisResult({ score, loading }: { score: number | null; loading: boolean }) {
  const t = useT();
