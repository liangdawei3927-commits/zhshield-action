  ...
  return (
    <svg width="150" height="150" viewBox="0 0 150 150" fill="none">
      ...
      <rect
        ...
        height="12"
        rx="4"
        fill="rgb(var(--zh-success) / 0.08)"
        stroke="rgb(var(--zh-success))"
        strokeWidth="2.5"
      />
      <path
        d="M52 60h46l-5 54a8 8 0 01-8 7H65a8 8 0 01-8-7l-5-54z"
        fill="rgb(var(--zh-success) / 0.08)"
        stroke="rgb(var(--zh-success))"
        strokeWidth="2.5"
      />
      <path
        d="M60 48v-8a15 15 0 0130 0v8"
        stroke="rgb(var(--zh-success))"
        strokeWidth="2.5"
        fill="none"
      />
      <path
        d="M64 72l3 32M75 72v32M86 72l-3 32"
        stroke="rgb(var(--zh-success))"
        strokeWidth="2"
        strokeLinecap="round"
      />
    ...
    <div className="flex items-center gap-4 mb-8">
      ...
      <div>
        ...
        <p className="text-sm text-zh-muted">
          {t('page.garbage.summary', {
            count: report.summary.garbageTotal,
            size: formatSize(report.summary.garbageSize),
          })}
        </p>
      </div>
      <PrimaryButton
        ...
        onClick={onRescan}
        loading={loading}
        loadingLabel={progressLabel || t('page.garbage.scanning')}
      >
...
export function CleanResultBanner({
  result,
  onRestore,
  restoring,
}: {
  result: GarbageCleanResultData;
  onRestore: () => void;
  restoring: boolean;
}) {
  const t = useT();
