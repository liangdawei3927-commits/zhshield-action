...
export function ShieldLightning() {
  return (
    <svg width="150" height="150" viewBox="0 0 150 150" fill="none">
      ...
      {/* 盾牌 */}
      <path
        d="M75 20L30 48v28c0 23.33 17.02 45 45 50 27.98-5 45-26.67 45-50V48L75 20z"
        fill="rgb(var(--zh-brand) / 0.06)"
        stroke="rgb(var(--zh-brand))"
        strokeWidth="2.5"
      />
      {/* 闪电 */}
      <path
        d="M75 48L58 78h12l-4 24 20-30H74l4-24z"
        fill="none"
        stroke="rgb(var(--zh-brand))"
        strokeWidth="3"
    ...
    <div className="flex items-center gap-4 mb-6">
      ...
      <div>
        ...
        <p className="text-sm text-zh-muted">
          {lastAt
            ? t('page.guard.lastScan', { time: new Date(lastAt).toLocaleString() })
            : t('page.guard.idleSubtitle')}
        </p>
      </div>
      <div className="ml-auto flex items-center gap-3">
        ...
        <PrimaryButton
          ...
          onClick={onRescan}
          loading={loading}
          loadingLabel={progressLabel || t('page.guard.scanning')}
        >
          {t('page.guard.scan')}
        </PrimaryButton>
      </div>
...
export function GuardLevels({ levels }: { levels: GuardLevelInfo[] }) {
  ...
  return (
    <div className="flex gap-4 mb-8">
      {levels.map((level) => {
        const st = STATUS_LABELS[level.status] ?? {
          textKey: 'page.guard.status.idle',
        ...
        return (
          <ResultCard key={level.level} variant="stats" className="flex-1">
            ...
            <div className="text-xs text-zh-muted mt-1">
              {level.status === 'idle'
                ? t('page.guard.status.idle')
                : t('page.guard.blockingCount', { count: level.blockingCount })}
            </div>
            {level.lastAt && (
...
export function GuardStats({ report }: { report: GuardReportData }) {
  ...
  return (
    <div className="flex gap-4 mb-8">
      {[
        {
          labelKey: 'page.guard.stats.totalChecks',
          value: report.summary.totalChecks,
          color: 'rgb(var(--zh-info))',
        },
        {
          labelKey: 'page.guard.stats.passed',
          value: report.summary.passed,
          color: 'rgb(var(--zh-success))',
        },
        {
          labelKey: 'page.guard.stats.warnings',
          value: report.summary.warnings,
          color: 'rgb(var(--zh-warning))',
        },
        {
          labelKey: 'page.guard.stats.blocked',
    ...
    return (
      <ResultCard variant="section" className="mt-8">
        <div className="flex items-center gap-3 mb-4">
          <h2 className="text-lg font-bold text-zh-ink">
            {t('page.guard.config.title', { defaultValue: '门禁配置' })}
          </h2>
        </div>
        <div className="text-sm text-zh-muted">
          {t('page.guard.config.loading', { defaultValue: '加载中...' })}
        </div>
      </ResultCard>
    );
  }
    ...
    <ResultCard variant="section" className="mt-8">
      ...
      <div className="space-y-4">
        <ConfigRow
          ...
          desc={t('page.guard.config.preCommitDesc', {
            defaultValue: '每次 git commit 前自动执行门禁检查',
          })}
          enabled={preCommit}
          onToggle={() => {
        ...
        <ConfigRow
          ...
          desc={t('page.guard.config.prePushDesc', {
            defaultValue: '每次 git push 前自动执行门禁检查',
          })}
          enabled={prePush}
          onToggle={() => {
            const next = !prePush;
        ...
        <ConfigRow
          ...
          desc={t('page.guard.config.blockOnCriticalDesc', {
            defaultValue: '当检测到严重级别问题时阻止提交操作',
          })}
