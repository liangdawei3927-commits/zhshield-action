...
function GuardRow({
  ...
  const sColor = check.severity
    ? SEVERITY_COLORS[check.severity]
    : { color: 'rgb(var(--zh-muted))', bg: 'rgb(var(--zh-muted) / 0.1)' };
  return (
...
export function GuardTable({
  ...
  onCopyToAi: (check: GuardReportData['checks'][number]) => void;
  onCopyAll: (checks: GuardReportData['checks'][number][]) => void;
  onReportFalsePositive: (check: GuardReportData['checks'][number]) => void;
}) {
  const t = useT();
      ...
      {failedChecks.length > 0 && (
        <div className="flex items-center justify-between px-5 py-3 border-b border-zh-line">
          <span className="text-xs text-zh-muted">
            {t('page.guard.copyAllPrompt', { count: failedChecks.length })}
          </span>
        ...
        <thead>
          <tr className="bg-zh-panel">
            <th className="text-left px-5 py-3 font-medium text-zh-muted">
              {t('page.guard.table.colCheck')}
            </th>
            <th className="text-left px-5 py-3 font-medium text-zh-muted">
              {t('page.guard.table.colSeverity')}
            </th>
            <th className="text-left px-5 py-3 font-medium text-zh-muted">
              {t('page.guard.table.colResult')}
            </th>
            <th className="text-right px-5 py-3 font-medium text-zh-muted">
              {t('page.guard.table.colDetail')}
            </th>
          </tr>
        </thead>
        <tbody>
