...
export function AmberBolt() {
  return (
    <svg width="150" height="150" viewBox="0 0 150 150" fill="none">
      ...
      {/* 闪电 */}
      <path
        d="M82 22L48 82h22l-8 46 40-64H78l8-42z"
...
export function PerformanceHeader({
  ...
  return (
    <div className="flex items-center gap-4 mb-8">
      ...
      <div>
        <h1 className="text-2xl font-bold text-zh-ink mb-1">
          {t('page.performance.header.title')}
        </h1>
        <p className="text-sm text-zh-muted">
          {t('page.performance.header.issueCount', { count: report.summary.total })}
        </p>
      </div>
      <PrimaryButton
        className="ml-auto"
        onClick={onRescan}
        loading={loading}
        loadingLabel={progressLabel || t('page.performance.detecting')}
      >
...
export function PerformanceScoreCard({ report }: { report: PerformanceReportData }) {
  ...
  return (
    <ResultCard variant="score" className="flex items-center gap-6 mb-6">
      <div className="w-20 h-20 rounded-full flex items-center justify-center bg-warning-50 border-[3px] border-warning-500 shrink-0">
        <span className="text-2xl font-bold text-warning-500">
          {report.summary.total === 0
            ? 100
            : Math.max(
                0,
                Math.round((1 - report.summary.total / (report.summary.total + 20)) * 100),
              )}
        </span>
      </div>
      <div>
        <div className="text-sm font-semibold text-zh-ink-2">
          {t('page.performance.score.health')}
        </div>
        <div className="flex gap-3 mt-2">
          <span className="text-xs flex items-center gap-1 text-warning-500">
            <span className="w-1.5 h-1.5 rounded-full bg-warning-500" />
            {t('page.performance.severity.medium')}{' '}
            {report.issues.filter((i) => i.severity === 'medium').length}
          </span>
          <span className="text-xs flex items-center gap-1 text-info-500">
            <span className="w-1.5 h-1.5 rounded-full bg-info-500" />
            {t('page.performance.severity.low')}{' '}
          ...
          <span className="text-xs flex items-center gap-1 text-success-700">
            ...
            {t('page.performance.score.fixable', { count: report.summary.autoFixable })}
          </span>
        </div>
      </div>
    </ResultCard>
...
function IssueCard({
  ...
  issue: PerformanceReportData['issues'][number];
  onCopyToAi: (issue: PerformanceReportData['issues'][number]) => void;
}) {
  const t = useT();
  ...
  return (
    <ResultCard>
      <div className="flex items-center gap-3">
        ...
        {issue.autoFixable && (
          <span className="px-1.5 py-0.5 rounded text-[10px] font-medium text-success-800 bg-success-50">
            {t('page.performance.autoFixable')}
          </span>
        )}
      ...
      {issue.suggestion && (
        <div className="mt-1 text-xs text-info-500">
          {t('page.performance.suggestion', { suggestion: issue.suggestion })}
        </div>
      )}
    </ResultCard>
  );
...
export function IssuesPanel({
  ...
  issues: PerformanceReportData['issues'];
  onCopyToAi: (issue: PerformanceReportData['issues'][number]) => void;
  onCopyAll: () => void;
}) {
  const t = useT();
      ...
      {issues.length > 0 && (
        <div className="flex items-center justify-between mb-3">
          <span className="text-xs text-zh-muted">
            {t('page.performance.copyAllHint', { count: issues.length })}
          </span>
