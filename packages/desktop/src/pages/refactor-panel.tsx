...
function SmellRow({ smell, onCopy }: { smell: Smell; onCopy: () => void }) {
  ...
  return (
    <BounceCard className="flex items-start gap-3 p-3 rounded-lg bg-zh-panel">
      <span className="px-1.5 py-0.5 rounded text-[10px] font-bold text-white shrink-0 mt-0.5" style={{ background: SEVERITY_COLORS[smell.severity] || 'rgb(var(--zh-muted))' }}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs mt-1 text-blue-700">{t('page.refactor.suggestion', { suggestion: smell.suggestion.description })}</p>
      </div>
...
export function SmellGroupPanel({ group, onCopyGroup, onCopyItem }: { group: SmellGroup; onCopyGroup: () => void; onCopyItem: (smell: Smell) => void }) {
  const t = useT();
  return (
    <ResultCard variant="score">
      <div className="flex items-center justify-between mb-3 gap-2 flex-wrap">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <span className="text-xs text-zh-muted shrink-0">{t('page.refactor.smellCount', { count: group.items.length })}</span>
