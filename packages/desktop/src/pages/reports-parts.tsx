    ...
    <ResultCard variant="score" className="flex gap-6 mb-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <div className="text-xs text-zh-muted mt-0.5">
    ...
    <div className="flex gap-4 mb-6">
      {dimensions.slice(0, 5).map((d) => (
        <ResultCard key={d.name} variant="stats" className="flex-1">
          ...
          <div className="text-lg font-bold" style={{ color: getScoreColor(d.score) }}>
            {d.score}
          </div>
          <div className="w-full h-1.5 rounded-full bg-zh-panel mt-2">
            <div
              className="h-full rounded-full"
              style={{ width: `${(d.weight || 0.5) * 100}%`, background: getScoreColor(d.score) }}
            />
          </div>
        </ResultCard>
      ))}
      ...
      {modules.map((m) => {
        ...
        return (
          <ResultCard key={m.path} variant="stats" className="min-w-[150px]">
            <div className="text-xs text-zh-muted mb-1 truncate" title={m.path}>
              {m.name}
            </div>
