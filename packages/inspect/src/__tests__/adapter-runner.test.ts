...
function makeAdapter(id: string, name: string, issues: Issue[] = [], shouldThrow = false): InspectAdapter {
  return {
    id,
    name,
    run: shouldThrow
...
describe('AdapterRunner', () => {
  it('should register and run a single adapter', async () => {
    ...
    const adapter = makeAdapter('eslint', 'ESLint', [
      { id: '1', ruleId: 'no-var', severity: 'error', category: 'quality', message: 'Use let', file: 'a.ts', autoFixable: true, source: 'eslint', fingerprint: 'f1' },
    ]);
  ...
  it('should return passed when adapter has no errors', async () => {
    const runner = new AdapterRunner();
    runner.register(makeAdapter('gitleaks', 'Gitleaks', [
      { id: '1', ruleId: 'R1', severity: 'info', category: 'security', message: 'Info', file: 'x.ts', autoFixable: false, source: 'gitleaks', fingerprint: 'f2' },
    ]));
  ...
  it('should run multiple adapters independently', async () => {
    ...
    runner.register(makeAdapter('b', 'AdapterB', [
      { id: '2', ruleId: 'R2', severity: 'warning', category: 'quality', message: 'Warn', file: 'b.ts', autoFixable: false, source: 'b', fingerprint: 'f3' },
    ]));
