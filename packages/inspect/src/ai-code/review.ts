...
export class AiCodeReviewImpl implements AiCodeReview {
  ...
  async complianceReport(project: ProjectProfile): Promise<AiComplianceReport> {
    ...
    const totalFiles = walkSourceFiles(projectPath).length;
    const aiCodeRatio = totalFiles === 0 ? 0 : markedFiles / totalFiles;

    // 风险分布：按模块（路径首段）聚合深度审查漏洞
