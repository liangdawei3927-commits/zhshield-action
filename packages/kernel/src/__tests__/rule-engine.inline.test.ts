...
describe('SopRuleEngine — 内联评估（pattern-scan / forbidden / threshold / layer-boundary）', () => {
  ...
  it('pattern-scan: 正则匹配敏感信息', async () => {
    ...
    registry.register(
      makeRule({
        ...
        content: {
          patterns: ['sk-[a-zA-Z0-9]{16,}', 'password\\s*[:=]\\s*["\']'],
        },
      }),
    );
  ...
  it('pattern-scan: 无违规时 passed', async () => {
    ...
    writeFileSync(
      ...
      ['const greeting = "hello";', 'const count = 42;'].join('\n'),
      'utf-8',
    );
  ...
  it('layer-boundary: 检测层违规导入', async () => {
    ...
    writeFileSync(
      ...
      ['import { UserService } from "domain/user-service";'].join('\n'),
      'utf-8',
    );
    ...
    writeFileSync(
      ...
      ['import { UserController } from "presentation/controller";'].join('\n'),
      'utf-8',
    );
