...
export class ProjectProfiler {
   ...
   */
  profileSync(projectRoot: string): ProfileResult {
  ...
  async profile(projectRoot: string): Promise<ProfileResult> {
    return this.profileSync(projectRoot);
  }

  private unknownProfile(projectRoot: string, reason: string): ProjectProfile {
    return {
      version: PROFILE_VERSION,
      projectRoot,
      language: 'unknown',
