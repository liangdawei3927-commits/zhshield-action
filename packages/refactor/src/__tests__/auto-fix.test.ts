...
describe('generateFix - inappropriate-intimacy', () => {
  ...
  it('returns null for a class with no public fields (no smell)', () => {
    const smell: CodeSmell = {
      ...
      suggestion: { type: '', description: '', priority: 'high', effort: 'small', autoFixable: true },
    };
...
describe('generateFix - callback-hell', () => {
  it('generates async keyword insertion for .then() chains', () => {
    const smell: CodeSmell = {
      ...
      context: { className: 'DataService', methodName: 'fetchData', metric: 'thenChainCount', value: 4, threshold: 3 },
      suggestion: {
...
describe('generateFixes', () => {
  it('filters only autoFixable smells and generates fixes', () => {
    const autoFixable: CodeSmell = {
      ...
      severity: 'error',
      message: '',
      location: { filePath: path.join(FIXTURES_DIR, 'intimacy.fixture.ts'), line: 1, column: 1, endLine: 7, endColumn: 1 },
    ...
    const nonAutoFixable: CodeSmell = {
      ...
      severity: 'warning',
      message: '',
      location: { filePath: path.join(FIXTURES_DIR, 'long-method.fixture.ts'), line: 1, column: 1, endLine: 1, endColumn: 1 },
      context: { className: 'OrderService', methodName: 'processOrder', metric: 'lineCount', value: 100, threshold: 80 },
      suggestion: { type: '', description: '', priority: 'medium', effort: 'medium', autoFixable: false },
    };
...
describe('generateFixes via RefactorEngine', () => {
  ...
  it('generates fixes for an intentional public-field smell', async () => {
    const original = 'export class PublicFields {\n  public name: string;\n  public age: number;\n  public email: string;\n  public address: string;\n  public phone: string;\n}\n';
  ...
  it('does not flag decorator-managed classes (DTO/entity) as inappropriate intimacy', async () => {
    ...
    try {
      ...
      const intimacySmells = report.files.flatMap(f => f.smells.filter(s => s.ruleId === 'inappropriate-intimacy'));
      expect(intimacySmells.length).toBe(0);
    } finally {
      try { fs.unlinkSync(decoratedFile); } catch { /* ignore */ }
    }
  });
});
...
describe('applyFixes', () => {
  ...
  it('applies edits to a temp file and restores content', () => {
    const original = 'export class Test {\n  public x: number;\n  public y: string;\n  public z: boolean;\n}\n';
    fs.writeFileSync(tmpFile, original, 'utf-8');
    ...
    const fixes = [{
      ...
      edits: [
        { filePath: tmpFile, startLine: 2, startColumn: 1, endLine: 2, endColumn: 24, replacement: '  private x: number;' },
        { filePath: tmpFile, startLine: 3, startColumn: 1, endLine: 3, endColumn: 23, replacement: '  private y: string;' },
        { filePath: tmpFile, startLine: 4, startColumn: 1, endLine: 4, endColumn: 24, replacement: '  private z: boolean;' },
      ],
    }];
  ...
  it('reports failure for non-existent file', () => {
    const fixes = [{
      ...
      ruleId: 'inappropriate-intimacy',
      description: '',
      edits: [{ filePath: '/tmp/nonexistent-dir-12345/test.ts', startLine: 1, startColumn: 1, endLine: 1, endColumn: 1, replacement: 'test' }],
    }];
