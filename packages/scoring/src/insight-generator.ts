...
export class InsightGenerator {
  generate(
    ...
    const tr = (key: string, params?: Record<string, unknown>) =>
      translate(key, locale ?? DEFAULT_LANGUAGE, params);
  ...
  private pushHealthInsight(
    ...
    const message =
      current >= 90
        ? tr('engine.scoring.insight.health.a')
        : current >= 75
          ? tr('engine.scoring.insight.health.b')
          : current >= 60
            ? tr('engine.scoring.insight.health.c')
            : tr('engine.scoring.insight.health.d');
    insights.push(message);
  }

  private pushVelocityInsight(
    insights: string[],
    velocity: number,
    tr: (key: string, params?: Record<string, unknown>) => string,
  ): void {
    const message =
      velocity > 2
        ? tr('engine.scoring.insight.velocity.rising', { velocity: velocity.toFixed(1) })
        : velocity < -2
          ? tr('engine.scoring.insight.velocity.declining', { velocity: velocity.toFixed(1) })
          : null;
    if (message) insights.push(message);
  }

  private pushVolatilityInsight(
    insights: string[],
    volatility: number,
    tr: (key: string, params?: Record<string, unknown>) => string,
  ): void {
  ...
  private pushStreakInsight(
    ...
    const dir =
      streak.direction === 'improving'
        ? tr('engine.scoring.insight.streak.improving')
        : streak.direction === 'declining'
          ? tr('engine.scoring.insight.streak.declining')
          : tr('engine.scoring.insight.streak.stable');
    insights.push(
      tr('engine.scoring.insight.streak.count', { direction: dir, count: streak.count }),
    );
  }

  private pushDecliningDimensionInsight(
    insights: string[],
    dimensionTrends: TrendReport['dimensionTrends'],
    tr: (key: string, params?: Record<string, unknown>) => string,
  ): void {
    ...
    insights.push(
      tr('engine.scoring.insight.decliningDimensions', {
        dimensions: declining.map((d) => d.name).join('、'),
      }),
    );
  }
}
