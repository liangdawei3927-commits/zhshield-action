...
export class SentinelController {
  ...
  handleWebhook(@Body() payload: AlertPayload): {
    accepted: boolean;
    eventId?: string;
    reason?: string;
  } {
  ...
  @HttpCode(HttpStatus.OK)
  startFileMonitor(@Body() body: { projectId: string; watchPaths: string[] }): { ok: boolean } {
  ...
  @HttpCode(HttpStatus.OK)
  startProcessMonitor(@Body() body: { projectId: string; command: string; cwd: string }): {
    ok: boolean;
  } {
  ...
  @HttpCode(HttpStatus.OK)
  startLogCollector(@Body() body: { projectId: string; logPaths: string[] }): { ok: boolean } {
